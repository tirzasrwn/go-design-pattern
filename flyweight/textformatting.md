...
In this code:
- `FormattedText` stores a boolean array for each character (memory-inefficient)
- `BetterFormattedText` uses `TextRange` objects only for formatted sections (flyweight)
- `formatting` slice only contains ranges that have formatting applied
- Multiple positions can share the same `TextRange` object
- More efficient when only small portions of text need formatting
