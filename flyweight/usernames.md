...
In this code:
- `User` stores full name as a string (duplicates "Jane" and "Doe" across users)
- `User2` uses flyweight: stores names as uint8 indices into `allNames` slice
- `getOrAdd()` reuses existing name strings instead of creating duplicates
- "Jane" and "Doe" are stored once in `allNames`, shared by all users
- Dramatically reduces memory when many users share common name parts
